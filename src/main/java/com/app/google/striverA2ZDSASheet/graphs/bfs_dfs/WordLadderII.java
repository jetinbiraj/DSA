...
//        List<String> wordList = List.of("hot", "dot", "dog", "lot", "log", "cog");
    ...
    //TODO: consider optimised approach here => https://www.youtube.com/watch?v=AD4SFl7tu7I&ab_channel=takeUforward
    ...
    public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
        ...
        while (!queue.isEmpty()) {
            ...
            int size = queue.size();
            Set<String> markRemoval = new HashSet<>();
            boolean found = false;

            for (int i = 0; i < size; i++) {
                var entry = queue.pollFirst();
                String last = entry.getLast();
                if (endWord.equals(last)) {
                    sequences.add(entry);
                    found = true;
                } else if (!found) {
                    checkAndAddNewEntries(queue, wordSet, entry);
                    markRemoval.add(last);
                }
            }
            ...
            wordSet.removeAll(markRemoval);
        }

        return sequences;
    }

    private void checkAndAddNewEntries(Deque<List<String>> queue, Set<String> wordSet, List<String> sequence) {

        String word = sequence.getLast();
        ...
        for (int i = 0; i < wordLength; i++) {
            ...
            for (char ch = 'a'; ch <= 'z'; ch++) {
                ...
                String setWord = wordSb.toString();

                if (wordSet.contains(setWord)) {
